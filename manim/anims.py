...
gs_tex = Tex("{{Any }}{{reasonable }}{{voting system }}{{sometimes }}{{incentivizes strategic voting. }}", color = text_color).scale(thm_scale)
gs_group = Group(gs_title, gs_tex).arrange_in_grid(cols = 1, cell_alignment=LEFT).to_edge(UP, buff = 0.5).to_edge(LEFT)
gs_new_tex = Tex(r"{{For any number of voters, any number of candidates, consider any voting system that satisfies: \\ }}" +
                    ...
                    r"There exists at least one scenario where the system incentivizes strategic voting. ", color = text_color
                    ).scale(thm_scale)
...
class Reasonable(Scene):
    def construct(self):
        ...
        line = Line(start = 10*LEFT, end = 10*RIGHT, color = text_color).next_to(gs_group, DOWN)
        ...
        table = VotingTable(["ABCD", "ABCD", "ABCD", "ABCD", "ABCD", 
                             "BCDA", "BCDA", "BCDA", "BCDA"])
        ...
        self.play(
            *[table[i].highlight("A") for i in range(5)]
        )
        self.wait()
        ...
        self.play(
            *[table[i].highlight("A") for i in range(5, table.num_of_voters)]
        )
        self.wait()

        self.play(
            *[table[i].highlight("B") for i in range(table.num_of_voters)]
        )
        self.wait()
        ...
        self.play(
            Circumscribe(Group(*gs_new_tex[1:3]), color = RED)
        )
        self.wait()
...
class Arrow(Scene):
    def construct(self):
        ...
        arrow_tex = Tex(r"{{Any reasonable voting system doesn't satisfy the }}{{\emph{independence of irrelevant alternatives}. }}").scale(thm_scale)
        arrow_full = Group(arrow_title, arrow_tex).arrange_in_grid(cols = 1, cell_alignment=LEFT).next_to(gs_group, DOWN, buff = 0.5).to_edge(LEFT)
        ...
        self.play(
            table.create_resulting_ranking("CBA")
        )
        self.wait()
        ...
        self.play(
            table.vysledne_poradi.highlight("B")
        )
        self.wait()
        self.play(
            table.vysledne_poradi.highlight("C")
        )
        self.wait()
        ...
        self.play(
            ...
            AnimationGroup(
            table[1].rearrange("ABC"),
            table.vysledne_poradi.rearrange("BAC")
            ),
            AnimationGroup(
        ...
        self.play(
            *[table[i].rearrange(example_table_str[i]) for i in range(len(example_table_str))]
        )
        ...
        self.play(
            Indicate(gs_tex[4], color = text_color)
        )
        self.wait()
        self.play(
            Indicate(arrow_tex[1], color = text_color)
        )
        self.wait()
...
class Approval(Scene):
    def construct(self):
        ...
        approval_data = [
            [ nothing.copy(),        monkey_img1.copy(),                       monkey_img2.copy(),                       monkey_img3.copy(),                         monkey_img1.copy(),  monkey_img1.copy(),         monkey_img1.copy(),          Tex("Sum"), ],
            [ avocado_img.copy(),    thumbsup_img.copy(),                      nothing.copy(),                           thumbsup_img.copy(),                        thumbsup_img.copy(), nothing.copy(),             thumbsup_img.copy(),         Tex("5"), ],
            [ banana_img.copy(),     thumbsup_img.copy(),                      nothing.copy(),                           thumbsup_img.copy(),                        nothing.copy(),      nothing.copy(),             nothing.copy(),              Tex("2"), ],
            [ coconut_img.copy(),    nothing.copy(),                           thumbsup_img.copy(),                      thumbsup_img.copy(),                        nothing.copy(),      nothing.copy(),             thumbsup_img.copy(),         Tex("3"), ],
        ]
        approval_table = Group(*[item for row in approval_data for item in row]).arrange_in_grid(rows = 4, buff = 0.1)

        self.play(
            Succession(
            *[AnimationGroup(*[FadeIn(approval_data[i][j]) for i in range(4)]) for j in range(len(approval_data[0]))]
            )
        )
        self.wait()
        ...
        for str1, str2 in [["img/thumbnail1.png", "10k likes"], ["img/thumbnail2.png", "10k likes"], ["img/thumbnail3.png", "10k likes"], ["img/thumbnail1.png", "10k likes"]]:
            ...
            videos_data.append(
                Group(Group(img, border), tex).arrange(DOWN)
            )        
        ...
        self.play(
            videos_group.animate.arrange(DOWN).shift(3*LEFT)
        )
        self.wait()
        ...
        self.play(
            Succession(
            *[Indicate(table[i]) for i in range(table.num_of_voters)],
            lag_ratio = 0.5
            )
        )
        self.wait()
        ...
        arrow = Tex(r"$\overset{?}{\rightarrow}$").scale(3).next_to(videos_group, RIGHT, buff = 1)
        ...
        for i in range(len(videos_group)):
            ...
            dislike.shift((videos_group[i].get_center()[1] - dislike.get_center()[1])*UP)
        ...
        self.play(
            Group(*[g for g in videos_group]).animate.arrange_in_grid(rows = 2)
        )
        self.wait()
        ...
        greater_tex = Tex(r"$<$").rotate(PI/2.0).move_to(videos_group[i1].get_center()/2.0 + videos_group[i2].get_center()/2.0)
        self.play(
            FadeIn(greater_tex)
        )
        self.wait()
        ...
        new_videos_group = Group(*[videos_group[i].target for i in [3, 2, 1, 0]]).arrange(DOWN).move_to(
            greater_tex.get_center()
        )
        self.play(
            FadeOut(greater_tex)
        )
        ...
        self.play(
            *[v[1].animate.set_color(text_color) for v in videos_group]
        )
        self.wait()
        ...
        self.play(
            videos_group.animate.move_to(ORIGIN)
        )
        self.wait()

        i_like = 1
        self.play(
            FadeIn(thumbsup_img.copy().next_to(videos_group[i_like]))
        )
        self.wait()
        
        self.play(
            *[FadeIn(thumbsup_img.copy().rotate(PI).next_to(videos_group[i])) for i in range(len(videos_group)) if videos_group[i].get_center()[1] > videos_group[i_like].get_center()[1]]
        )
        self.wait()
...
class Debriefing(Scene):
    def construct(self):
        ...
        self.play(
            Circumscribe(
                Group(*[voting_data[i][2] for i in range(1, len(voting_data))]), color = RED
            )
        )
        self.wait()
        ...
        bush_gore_table = Group(
            ...
            Tex("Nader:"),
            Tex("97\,488"),
        ).arrange_in_grid(cols = 2).shift(2*DOWN)

        self.play(
            FadeIn(bush_gore_table)
        )
        self.wait()
        
        self.play(
            FadeOut(bush_gore_table)
        )
        self.wait()
